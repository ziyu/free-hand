...
先点 **重新检测**。现在会同时检查当前进程的系统信任状态、实际外部窗口引用
读取、键鼠输入权限；不再把一个 `false` 简单说成“你没有开启”。目标应用暂时
不响应或没有窗口，也不会被当成没有授权。返回应用、切换目标和系统唤醒时会
重新检查；授权变化不会自动发送草稿。

如果重新打包后旧条目不对应当前版本，点 **连接当前版本**，在“诊断详情”里
点 **定位当前应用**；在系统辅助功能列表移除旧 Free Hand 条目，再用 **+**
添加这份应用并开启。开发签名的哈希会随代码更新改变，应用名称相同并不表示
旧授权仍适用。使用稳定证书签名才能在更新之间维持签名身份。程序不会修改
系统权限数据库、切换证书或伪造“已授权”。
